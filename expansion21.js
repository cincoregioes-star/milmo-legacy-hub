...
function sanitizeVisibleSupabase(root=document.body){
  if(!root||root.dataset.mlhSanitized==='1')return;
  const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
  nodes.forEach(n=>{let t=n.nodeValue||'',v=t;v=v.replace(/BANCO REAL NO SUPABASE/gi,'CATÁLOGO HISTÓRICO VERIFICADO');v=v.replace(/Métricas agregadas carregadas do Supabase\./gi,'Métricas agregadas carregadas.');v=v.replace(/Supabase indisponível\./gi,'Serviço temporariamente indisponível.');v=v.replace(/\bSupabase\b/gi,'Banco');if(v!==t)n.nodeValue=v});
  root.dataset.mlhSanitized='1';
}
...
async function loadSearchData(){
  if(!client)return;const [a,m]=await Promise.all([client.from('milmo_areas').select('slug,name,adventure,description').order('name'),client.from('missions').select('slug,title,area_slug,npc,objective,reward').order('title')]);areas=a.data||[];missions=m.data||[];enhanceSearch();
}
...
function currentMatches(q){
  const n=norm(q),adv=activeAdventure();if(n.length<2)return {area:[],mission:[],areaTotal:0,missionTotal:0};const allowed=a=>adv==='Todos'||a.adventure===adv;
...
function openAreaFromSearch(slug){
  const a=areas.find(x=>x.slug===slug);if(!a)return;const tab=$$('[data-mp-adv]').find(x=>x.dataset.mpAdv==='Todos');if(tab&&!tab.classList.contains('on'))tab.click();const input=$('#mpSearch');if(!input)return;input.value=a.name;input.dispatchEvent(new Event('input',{bubbles:true}));setTimeout(()=>{const btn=$(`[data-mp-area="${CSS.escape(slug)}"]`);if(btn)btn.click()},100);
}
function renderStrictResults(){
  const input=$('#mpSearch'),box=$('#strictSearchResults');if(!input||!box)return;const q=input.value.trim();if(q.length<2){box.classList.remove('on');box.innerHTML='';return}const r=currentMatches(q);box.classList.add('on');const items=[];
  ...
  box.innerHTML=`<div class="strict-search-head"><b>Busca por ilha ou missão</b><span class="tag">${r.areaTotal} área(s) • ${r.missionTotal} missão(ões)</span></div>${items.length?`<div class="strict-search-list">${items.join('')}</div>`:'<div class="strict-search-empty">Nenhuma ilha ou missão encontrada com esse nome.</div>'}`;$$('[data-strict-area]').forEach(b=>b.onclick=()=>openAreaFromSearch(b.dataset.strictArea));
}
function enhanceSearch(){
  const input=$('#mpSearch');if(!input||input.dataset.strictSearch==='1')return false;input.dataset.strictSearch='1';input.placeholder='Buscar ilha, área ou missão...';input.setAttribute('aria-label','Buscar ilha, área ou missão');const toolbar=input.closest('.mp-toolbar');if(toolbar&&!$('#strictSearchResults')){const help=document.createElement('small');help.className='strict-search-help';help.textContent='Digite o nome de uma ilha/área ou de uma missão. Também busca NPC e objetivo.';toolbar.after(help);const box=document.createElement('div');box.id='strictSearchResults';box.className='strict-search-results';help.after(box)}input.addEventListener('input',()=>setTimeout(renderStrictResults,0));input.addEventListener('keydown',ev=>{if(ev.key==='Escape'){input.value='';input.dispatchEvent(new Event('input',{bubbles:true}))}});$$('[data-mp-adv]').forEach(b=>b.addEventListener('click',()=>setTimeout(renderStrictResults,0)));return true;
}
function boot(){
  fixStaticLabels();let ticks=0;const maintenance=setInterval(()=>{fixBossBeach();if(areas.length)enhanceSearch();if(++ticks>=24)clearInterval(maintenance)},500);
  let tries=0;const searchWait=setInterval(()=>{if($('#mpSearch')){clearInterval(searchWait);loadSearchData()}else if(++tries>40)clearInterval(searchWait)},250);
}
